...
指针变量作为函数形参和数组作为函数形参`void func(int arr[50]);`，这里的50是无效的。上述代码其实等价于`void func(int arr[]);`
在二维数组中，数组的第一维的数字是无效的，会被编译器忽略，比如`void func(int arr1[2][1]);`
...
double *b;
```
`a ++`和`b++`，指针移动的字节数量不一样。a移动的数量是`sizeof(int)`，b移动的数量是`sizeof(double)`
...
`int *integers[]`和`int **integers`的区别是：`int *integers[]`是一个指针数组，每个元素都是一个整数指针，而`int **integers` 是一个指向整数指针的指针。以下代码说明二者的联系。
```
int num1 = 10, num2 = 20, num3 = 30, num4 = 40;
...
然而，在函数外部使用`*p=9`对函数地址进行了内存改写操作，这样是错误的。会导致意想不到的结果，进而导致程序崩溃。究其原因就是，
